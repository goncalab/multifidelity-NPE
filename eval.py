...
import pandas as pd
from mf_npe.one_experiment import run_evaluation
...
##### USAGE #####
# python eval.py --models_to_run npe mf_npe --simulator_task GaussianBlob --lf_datasize 1000 --hf_datasize 100 --n_true_xen 

def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate trained multifidelity models.')
    ...
    parser.add_argument('--eval_metric', type=str, nargs='+', required=True, help='evaluation metric. For toy tasks, use: c2st, wasserstein, mmd, nltp. For others: nltp')
...
def main():
    ...
    # Sort the strings so that the order of models does not matter when loading the model
    ...
    # For eval metrics
    for eval_metric in args.eval_metric:
        ...
        for net_i in range(args.n_net_inits):
            ...
            mf_abc_weights = train_data.get('mf_abc_weights', None)  # Use .get() to avoid KeyError if not present
            ...
            # Run evaluation
            df_results = run_evaluation(task_setup, 
        ...
        if seed_max == args.seed:
            name = f"evaluate_{eval_metric}_{models_str}_LF{lf_str}_HF{hf_str}_Ninits{net_init}_seed{args.seed}.pkl"
        else:
            name = f"evaluate_{eval_metric}_{models_str}_LF{lf_str}_HF{hf_str}_Ninits{net_init}_seed{args.seed}-{seed_max}.pkl"
            
        dump_pickle(main_path, name, batch_df_results)
        ...
        plot_methods_performance_paper(grouped_df, f"{task_setup.sim_name}_({task_setup.config_data['theta_dim']} dims, avg over {net_init} networks)", task_setup.batch_lf_sims, df_results['evaluation_metric'][0], task_setup) 
