...
class OUprocessXinverseNoise(Prior):
    def __init__(self, config_data, gamma, mu_offset, noise):
    ...
    def summary_statistics(self, n_simulations, prior):
        ...
        # Invert the subsamples
        print("Before flip sum", subsampled_simulations)
        subsampled_simulations = subsampled_simulations.flip(dims=[1])    
        print("after flip sum", subsampled_simulations)
        
        # Invert trace as well
        simulations = simulations.flip(dims=[1])
    ...
    def simulator(self, thetas):        
        ...
        # Invert the subsamples
        print("Before flip sim", subsampled_simulations)
        subsampled_simulations = subsampled_simulations.flip(dims=[1])    
        print("after flip sim", subsampled_simulations)
    ...
    def true_log_likelihood_one_theta_multiple_x(self, theta, true_xs) -> torch.Tensor:
        """
        Adapted to: One theta, multiple xs
        - theta: shape (theta_dim,)
        - true_xs: shape (N_x, x_dim)
        """
        mu = theta[0]
        sigma = theta[1]
        gamma = theta[2] if self.theta_dim in [3, 4] else self.gamma
        mu_offset = theta[3] if self.theta_dim == 4 else self.mu_offset
        
        N_x = true_xs.shape[0]
        x_dim = true_xs.shape[1]
        ...
        # First term (xto)
        xto_dist = self.xt0_dist(mu_offset, mu)
        xto = true_xs[:, 0]  # shape (N_x,)
        xto_log_prob = xto_dist.log_prob(xto)  # shape (N_x,)
        logsum = xto_log_prob

        for j in range(1, x_dim):
            g = (1 - torch.exp(-2 * gamma * self.dt * (idx[j]-idx[j-1]))) / gamma
            
            x_prev = true_xs[:, j-1]  # shape (N_x,)
            x_curr = true_xs[:, j]    # shape (N_x,)

            term = 1 / (torch.sqrt(torch.pi * g) * sigma)  # scalar
            exp1 = -(1 / (g * sigma**2))  # scalar
            exp2 = (mu - x_curr) - torch.sqrt(1 - gamma * g) * (mu - x_prev)  # shape (N_x,)
            exp = torch.exp(exp1 * exp2**2)  # shape (N_x,)

            logsum += torch.log(term * exp)  # shape (N_x,)

        return logsum  # shape (N_x,)
