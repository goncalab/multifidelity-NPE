...
class SIR_lowres(Prior):
    ...
    def __init__(self, config_data):
        ...
        # measure each 4 days, not each day (decreased temporal resolution)
        self.saveat: float = config_data.get('saveat', 32.0)  # was 1.0
        ...
        # pick EXACTLY 10 indices evenly over [0, num_steps-1], regardless of num_steps
        self.sub_idx = self._make_sub_idx(self.num_steps, n_points=10)
    ...
    @staticmethod
    def _make_sub_idx(num_steps: int, n_points: int) -> torch.Tensor:
        if num_steps < n_points:
            # fall back to step=1 and truncate; guarantees n_points if possible
            idx = torch.linspace(0, num_steps - 1, steps=num_steps).round().long()
            # pad by repeating last index (rare, only if timeline too short)
            pad = torch.full((n_points - num_steps,), num_steps - 1, dtype=torch.long)
            return torch.cat([idx, pad])
        # Evenly spaced integer indices from 0 to num_steps-1 (inclusive)
        return torch.linspace(0, num_steps - 1, steps=n_points).round().long().unique()
    ...
    def summary_statistics(self, n_simulations, prior):
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig("./mf_npe/simulator/task7/images/subsampled_sir_lowres.png", dpi=300)
            plt.savefig("./mf_npe/simulator/task7/images/subsampled_sir_lowres.pdf", dpi=300)
            plt.close()
        except Exception:
            pass
